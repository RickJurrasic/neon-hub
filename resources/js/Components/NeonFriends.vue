...
const removeLink = async (id) => {
    try {
        ...
        // Případně pokud pro to používáš přímý endpoint, zkontroluj jeho URL
        // (často bývá /friendships/{id}).
...
<template>
    ...
    >
        <div
            class="flex justify-between mb-6 border-b border-purple-500/20 pb-2"
        >
            <span class="text-purple-400 text-xs tracking-tighter"
                >>> NEON_NETWORK_ENTITIES</span
            >
            <button
                type="button"
                class="text-[9px] text-purple-700 underline active:text-white py-1 px-2 -mr-2 select-none"
                @click="$emit('back')"
            >
                TERMINATE_LINK
            </button>
        ...
        <div class="space-y-4 overflow-y-auto no-scrollbar grow">
            ...
            >
                ...
                >
                    ...
                    >
                        <span
                            :class="
                                !req.read
                                    ? 'animate-pulse text-purple-300 shadow-[0_0_10px_#a855f7]'
                                    : ''
                            "
                        >
                            ID
                    ...
                    <div class="flex flex-col">
                        <span
                            class="text-white text-xs uppercase tracking-wider"
                        >
                            {{ req.name }}
                        </span>
                        <span class="text-[8px] text-purple-400/60"
                            >REQUEST_PENDING</span
                        >
                    </div>
                </button>
                ...
                >
                    <template v-if="decliningRequestId === req.id">
                        <span
                            class="text-[8px] text-red-500 animate-pulse font-bold self-center"
                        >
                            DECLINE?
                        </span>
            ...
            <div v-if="friends.length > 0" class="mt-6">
                <div
                    class="text-purple-400 text-xs pb-2 border-b border-purple-500/20"
                >
                    >> ACTIVE_LINKS
                </div>
                ...
                >
                    ...
                    >
                        <div class="flex items-center gap-4">
                            <div
                                class="w-2 h-2 rounded-full bg-purple-500 shadow-[0_0_8px_#a855f7] animate-pulse"
                            ></div>
                            <span
                    ...
                    <div
                        v-if="unlinkingFriendId === friend.id"
                        class="flex items-center gap-2 shrink-0"
                    >
                        <span
                            class="text-[8px] text-red-500 animate-pulse font-bold"
                        >
                            UNLINK?
                        </span>

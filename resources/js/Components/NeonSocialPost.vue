...
onMounted(() => {
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            // Jakmile jednou post spatříme, necháme ho true,
            // aby při rychlém scrollu divoce neblikal a neztrácel výšku
            if (entry.isIntersecting) {
                isVisible.value = true;
            }
        });
    }, {
        threshold: 0.05, // Výrazně sníženo, stačí letmý dotyk s viewportem
        rootMargin: '100px 0px 300px 0px' // Načte post s předstihem, než k němu dojedeme
    });
...
<template>
    <article ref="cardRef"
        class="neon-panel-wrapper w-full rounded-[2rem] snap-start transition-all duration-[700ms] ease-out min-h-[150px]"
        :class="[isVisible ? 'opacity-100 scale-100 translate-y-0' : 'opacity-20 scale-95 translate-y-6']"
        ...
        <div class="relative w-full">
            ...
            <div
                ...
                <div
                    class="w-full flex justify-between items-center mb-6 md:mb-8 border-b border-white/10 pb-4 md:pb-6">
                    <div class="flex items-center gap-4 md:gap-6">
                        <div
                            class="w-3 h-3 md:w-4 md:h-4 bg-sky-500 shadow-[0_0_15px_#3b82f6] rounded-full animate-pulse">
                        </div>
                        <span
